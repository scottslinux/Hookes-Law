...
Slider::Slider(Vector2 loc, float sliderscale,int detnts,int minim,int maxi, std::string labeltxt)
{
...
void Slider::draw()
{
   ...
   char buffer[100];
            snprintf(buffer,sizeof(buffer),"%s %d",sliderlabel.c_str(),value);

            DrawTextEx(pencil,buffer,{location.x+plate_off.width*scale+10,
                    location.y+(plate_off.height*scale)/3},50,0,WHITE);
